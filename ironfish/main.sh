...
install_ironfish() {
    ...
    *)
    ...
    docker run -itd --name $Container_name --net host --volume /root/.node:/root/.ironfish ghcr.io/iron-fish/ironfish:latest start
    sleep 5
    docker exec -it $Container_name bash -c "ironfish config:set blockGraffiti ${name}"
    docker exec -it $Container_name bash -c "ironfish config:set enableTelemetry true"
    echo "启动成功！"
}
...
stop_ironfish() {
    docker exec -it $Container_name bash -c "ironfish stop"
    sleep 5
    echo "停止成功！"
}
...
mine_ironfish() {
    ...
    docker exec -itd $Container_name bash -c "ironfish miners:start"
}

send_ironfish() {
    echo "发送转账请在节点同步完后进行,否则会提示错误"
    echo "流程请参考https://ironfish.network/docs/onboarding/send-receive-iron-fish-transactions"
    docker exec -it $Container_name bash -c "ironfish wallet:send"
}

creat_ironfish() {
    echo "正在创建钱包"
    docker exec -it $Container_name bash -c "ironfish wallet:create"
    echo "新钱包创建成功，挖矿时请记得更改为你需要使用的钱包"
}

set_ironfish() {
    echo "正在导出所有的本地钱包名字↓"
    docker exec -it $Container_name bash -c "ironfish wallet:accounts"
    read -p " 请输入你想使用的钱包名字（如上）:" name
    docker exec -it $Container_name bash -c "ironfish wallet:use ${name}"
...
export_ironfish() {
    ...
    docker exec -it $Container_name bash -c "ironfish wallet:export ${name}"
    echo "成功导出 ${name} 钱包,请备份"
}

import_ironfish() {
    echo "请输入导入的 钱包名字 和 spendingKey (这些信息在导出时提供)"
    docker exec -it $Container_name bash -c "ironfish wallet:import"
}

balance_ironfish() {
    echo "正在显示钱包余额,请确保节点已同步完成并领水...."
    docker exec -it $Container_name bash -c "ironfish wallet:balance"
}

read_ironfish() {
    echo "正在检查实时状态，如需退出请按 CTRL+C"
    docker exec -it $Container_name bash -c "ironfish status -f"
}
...
faucet_ironfish() {
    ...
    docker exec -it $Container_name bash -c "ironfish faucet"
}
